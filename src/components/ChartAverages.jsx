...
import { BREADTH_TOP_RATIO, CAREER_BREADTH_WEIGHT, CAREER_PEAK_WEIGHT, FE_UI, FEATURE_SCORES_SETTINGS } from "@/lib/constants";
import { computeAverages, formatAvgScore } from "@/lib/scores";
import { cn } from "@/lib/utils";

import { useAppStore } from "@/store/useAppStore";

function AvgCard({ label, value, sub, className, title, labelPx, valuePx, subPx }) {
  return (
    <div
      title={title}
      className={cn(
    ...
    >
      ...
      <span className="font-extrabold tabular-nums" style={{ fontSize: valuePx }}>
        {value}
      </span>
      {sub ? (
...
export function ChartAverages({ chartWidth = 0 }) {
  ...
  const { breadth, human, effective, career } = computeAverages(levels, trackVariant);
  ...
  return (
    <div
      ...
      className="mb-3 grid w-full grid-cols-4 gap-2 sm:gap-4 last-of-type:mb-0"
      aria-label="Breadth, strength, effective score, and seniority"
    >
      <AvgCard
        label="Breadth"
