...
export function LatestChangesToggle({ show, onToggle }) {
  return (
    <button
      ...
      aria-checked={show}
      onClick={onToggle}
      className="group inline-flex shrink-0 cursor-pointer items-center gap-1.5 rounded-full border border-slate-300 bg-white py-1 pl-2.5 pr-1.5 text-[11px] font-semibold tracking-wide text-slate-600 hover:bg-slate-50 hover:text-slate-800 sm:text-[12px]"
    >
